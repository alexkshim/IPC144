...
void search(const struct Item item[], int NoOfRecs) {
	//int checkLimit = 0;			// to hold the temporary value to check if over or less than the limit
	...
	// /* long */
	//int exitWhile = 0;	// to exit the while loop
	//do {
	//	printf("Please enter the SKU: ");
	//	scanf("%d", &checkLimit);
	//	if (checkLimit <= SKU_MAX && checkLimit >= SKU_MIN) {
	//		exitWhile++;
	//	}
	//} while (exitWhile != 1);
	...
	//	printf("Please enter the SKU: ");
	//	scanf("%d", &checkLimit);
	//} while (!(checkLimit <= SKU_MAX && checkLimit >= SKU_MIN));
...
void adjustQuantity(struct Item item[], int NoOfRecs, int stock) {
	...
	if (checkItem != 1) {
		...
		scanf("%d", &zeroToAbort);	//Then check the value entered by the user
	...
	//if (item[tempIndex].quantity <= item[tempIndex].minQuantity) {
